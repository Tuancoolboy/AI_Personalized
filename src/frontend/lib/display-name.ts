...
/** Xưng hô trong lời chào mở đầu — luôn dùng tên nếu biết (Hải / chị Lan / a Minh). */
export function buildGreetingAddress(
...
/** Xưng hô thân thiện trong thân bài (neutral → "bạn"). */
...
/** Coach chat: chào trung tính là "bạn"; "anh"/"chị" chỉ khi đã chọn. */
export function buildCoachAddresses(
  fullName: string,
  preferredAddress: PreferredAddress = "neutral",
): { named: string; casual: string } {
  if (preferredAddress === "neutral") {
    return {
      named: "bạn",
      casual: "bạn",
    };
