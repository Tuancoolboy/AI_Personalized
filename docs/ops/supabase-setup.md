...
37. `supabase/migrations/20260622054535_allow_cross_org_hoc_tap_room_join.sql` — migration lịch sử từng mở preview/join room bằng mã giữa các công ty
38. `supabase/migrations/20260623023753_hoc_tap_real_xp_audience.sql` — XP quiz thật, audience Cộng đồng/Công ty và thu hồi cross-company room access
...
Migration `20260623023753_hoc_tap_real_xp_audience.sql` thay thế hành vi
cross-company của migration `20260622054535`. Account không có
`organization_members` cùng dùng không gian **Cộng đồng AI Trợ Lý**; account có
membership chỉ thấy room trong công ty đó. Biết đúng mã phòng của audience khác
vẫn nhận `ROOM_NOT_FOUND`; muốn chuyển từ Cộng đồng vào công ty phải chấp nhận
link mời hợp lệ.

Nếu hai account cùng audience vẫn nhận `404` sau khi deploy code, chạy:
...
và xác nhận migration `20260623023753` đã có trên đúng project Supabase mà
Vercel đang dùng.
